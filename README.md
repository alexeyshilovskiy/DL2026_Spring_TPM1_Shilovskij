...
Book24.ru - это интернет-магазин издательской группы Эксмо-АСТ. На их сайте продаются книги эздателей таких как «Эксмо» и «АСТ», «Манн, Иванов и Фербер», «Азбука», «Альпина», «Синдбад».
А также они взаимодействуют с «БОМБОРА», «Corpus», «Mainstream», «Редакция Елены Шубиной», «Вилли-Винки», «Fanzon», «Комильфо» и «Канц-Эксмо». 
Они также являются издателями книг и поэтому знают когда выходят новые книги и получаем их первыми, что позваляет им формировать свои цены на дынную продукцию.

**Основная функция:** Предоставить максимально широкий и удобный канал для поиска, выбора и покупки книг(печатынй вариант и аудиокниги), канцелярии и других товаров с удобной доставкой по всей России, используя инструменты лояльности и контентной поддержки (обзоры, рейтинги, подборки).
Доставка осуществялется на свои пункты выдачи, почтой, а также можно получить из рук курьера.
...
**Блок-схема процесса**
![Блок-схема процесса покупки книги](pm/purchase_flow_book24.png)
*(Файл схемы в формате PNG/ссылка на draw.io добавлена в папку `/pm`)*

**Пошаговое пояснение к схеме:**
...
4.  **Проверка наличия результатов:**
    *   *Альтернативный путь (Ничего не найдено):* Пользователь видит сообщение «К сожалению, по вашему запросу «ЗАПРОС» мы ничего не нашли.», под этим сообщение он видит «Но, возможно, вам будут интересны эти товары:», где ему предложат похожие запросы. Также на этом сайте допустив ошибку в названии книги, будет отображён текст который я описал ранее о том что ничего не было найдена, ведь на сайте не была добавлена корректура текста. Хотя в предложеных книгах снизу она скорее всего будет отображена, смотреть ![скриншот](other/screenshot_from_books.jpg). Но если допустить опечатку на авторе то скорее всего сайт опустит слово с ошибкой и выведет всех авторов у которых имеются похожее имя или фамилия как указанно на ![скриншоте](other/screenshot_about_author.jpg). Иначе если не находим товар или автора то вовращаемся к шагу 2.
...
6.  **Выбор книги и просмотр карточки:** Пользователь кликает на книгу, видит подробное описание(цену, хараетеристику, обложку), отзывы и оценки(пользовательские и рейтинг экспертов), информацию о наличии.
...
10.  **Оформление заказа:** Кликнув «Перейти к оформлению» пользователь заполняет данные для доставки, выбор способа оплаты и доставки. 
    *   *Альтернативный путь (Пользователь не авторизировался):* Если пользователь не авторизирован, то сайт не даст ему оформить заказ поэтому пользователь регестрируется или заходит в уже зарегестрированный аккаунт.
